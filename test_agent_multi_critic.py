...
# 在母体中，创建一个校对智能体
agent_校对者 = nest.create_agent(
    agent_id=f"校对者",
    agent_class="agent_matrix.agent.agent_basic_qa->BasicQaAgent",
    agent_kwargs={"sys_prompt":
        "你是一个校对者，你的任务是校对文本，找出其中的错误。你的队伍中还有一个润色者和一个中文本地化者，你只需要做好自己的分内工作，不能僭越其他人的工作。",
...
# 在母体中，创建一个润色智能体
agent_kwargs = {""}
agent_润色者 = nest.create_agent(
    agent_id=f"润色者",
    agent_class="agent_matrix.agent.agent_basic_qa->BasicQaAgent",
    agent_kwargs={"sys_prompt":
        "你是一个润色者，你的任务是润色文本。你的队伍中还有一个校对者和一个中文本地化者，你只需要做好自己的分内工作，不能僭越其他人的工作。",
        "query_construction": "Do your job according to the instructions."
    }
)

# 在母体中，创建一个中文专业化智能体
agent_本地化 = nest.create_agent(
    agent_id=f"本地化",
    agent_class="agent_matrix.agent.agent_basic_qa->BasicQaAgent",
    agent_kwargs={"sys_prompt":
        "你是一个中文本地化专者，你的任务是将不符合中文语言习惯的句子进行修改。你的队伍中还有一个校对者和一个润色者，你只需要做好自己的分内工作，不能僭越其他人的工作。",
        "query_construction": "Do your job according to the instructions."
    }
)

# 在母体中，创建一个中文专业化智能体
agent_总结者 = nest.create_agent(
    agent_id=f"总结者",
    agent_class="agent_matrix.agent.agent_basic_qa->BasicQaAgent",
    agent_kwargs={"sys_prompt":
        "你的任务是将校对者、润色者和本地化者的工作结果进行汇总，形成最终的文本。你不得输出除最终文本之外的任何废话。",
        "query_construction": "Do your job according to the instructions."
    }
)

# 用明确的边连接定义智能体的交互
agent_校对者.create_edge_to(dst_agent_id=agent_润色者)
agent_润色者.create_edge_to(dst_agent_id=agent_本地化)
agent_本地化.create_edge_to(dst_agent_id=agent_总结者)
